...
def get_nuke_exe_path(full_path='', nuke_dir='', nuke_exe=''):
	...
	_fl_error_check.file_exists(nuke_exe)
	return nuke_exe


def __do_return_full_path(full_path, check_f=None):
	if check_f is None:
		check_f = _fl_error_check.file_readable
...
def get_nk_script_path(full_path='', nk_dir='', nk_filename=''):
	...
	_fl_error_check.dir_exists(nk_dir)
	...
	_fl_error_check.file_readable(nk_filename)
	return nk_filename
...
def get_py_script_path(full_path='', py_dir='', py_filename=''):
	...
	_fl_error_check.dir_exists(py_dir)
	...
	_fl_error_check.file_readable(py_filename)
	return py_filename
...
def get_src_tex(src_tex):
	...
	if isinstance(src_tex, _str_t):
		_fl_error_check.file_readable(src_tex)
